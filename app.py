...
# 1. Configuração para o Samsung A56 (Visual no PC também)
st.set_page_config(page_title="Monitor Parkinson Pro", layout="centered")

# --- CONFIGURAÇÃO DOS LINKS (COLE OS SEUS AQUI) ---
URL_PLANILHA = "COLE_AQUI_O_LINK_DE_COMPARTILHAMENTO_DA_PLANILHA"
URL_POST = "COLE_AQUI_A_URL_DO_APPS_SCRIPT_QUE_VOCE_COPIOU"

def ler_dados():
    try:
        # Transforma o link da planilha em link de download CSV para leitura
...
st.markdown(f"""
<style>
    .stButton > button {{ height: 90px !important; font-size: 20px !important; font-weight: bold !important; border-radius: 20px; }}
</style>
...
with aba_reg:
    ...
    if st.session_state.menu == "Med":
        ...
        if st.button("💾 CONFIRMAR MEDICAMENTO"):
            ...
            dados = {"Data": agora.strftime("%d/%m/%Y"), "Cat": "Medicação", "Ini": h, "Fim": h + 1.0, "Desc": ", ".join(remedios)}
            requests.post(URL_POST, json=dados)
        ...
        if st.button("💾 CONFIRMAR COMIDA"):
            agora = hora_sp()
            h = agora.hour + agora.minute/60
            dados = {"Data": agora.strftime("%d/%m/%Y"), "Cat": "Alimentação", "Ini": h, "Fim": h + 1.0, "Desc": comida}
            requests.post(URL_POST, json=dados)
            st.session_state.menu = None
            st.rerun()
